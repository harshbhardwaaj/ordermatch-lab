...
- `/prototype/orders`: full order log across every order, sample or your own, with a self-serve reset. There's no login, but each visitor's browser gets its own isolated copy of the demo data (a cookie, not an account) — nobody sees another visitor's orders
